...
def mask_image(label, boxes, img):
    mask_path = os.getenv("TRAIN_Y_MASKED")
    new_image = np.zeros(shape=(256, 256, 3), dtype=np.float32)

    for i in boxes:  # Mask all regions except bounding box
        new_image[i[1]:i[3], i[0]:i[2]] = img[i[1]:i[3], i[0]:i[2]]

    cv2.imwrite(mask_path+label+'.png', new_image)
